const logo = `<svg class="oissu-logo" width="1093" height="421" viewBox="0 0 1093 421" fill="none" xmlns="http://www.w3.org/2000/svg">
...
(function(oissu, $, undefined) {
    ...
    oissu.bubbleToggle = function() {
        ...
        oissu.config["bubble"] = !oissu.config["bubble"];
        updateConfig();
    };

    oissu.imageToggle = function() {
        $("body").toggleClass("oissu-image");
        oissu.config["image"] = !oissu.config["image"];
        updateConfig();
    };
    ...
    oissu.chatToggle = function() {
        if ($("body").hasClass("oissu-custom")) {
            ...
            oissu.config["bubble"] = false;
            oissu.config["image"] = false;
            oissu.config["custom"] = false;
        } else {
            $("body").toggleClass("oissu-image");
            $("body").toggleClass("oissu-bubble");
            oissu.config["bubble"] = !oissu.config["bubble"];
    ...
    oissu.fullscreenToggle = function(oissuEmbed) {
        ...
        tippy.createSingleton(
            tippy(
                $(oissuEmbedFullscreen)
                    .find(".os-toolbar .tippy")
                    .toArray()
            ),
            {
    ...
    function initNotes(oissuEmbed) {
        noteInline = $(oissuEmbed).find("sup");
        noteList = $(".os-note").children();
        noteInline.each(function() {
            ...
            $(note).attr("id", `os-note-${noteNumber}`);
            const noteLinked = $(`<a class="os-note-inline"></a>`)
    ...
    function initNotice(oissuEmbed) {
        ...
        noticeBlockquote = $(oissuEmbed)
            .find("[data-notice-type]")
            .last();
        noticeContent = $(noticeBlockquote).html();
        noticeType = $(noticeBlockquote).attr("data-notice-type");
        noticePrompt = $(noticeBlockquote).attr("data-notice-prompt");
    ...
    function initToolbar(oissuEmbed) {
                ...
                $(navbar).append(
                    $("<a></a>")
                        .attr({
                            ...
                            href: directory.substring(0, directory.indexOf(" "))
                        })
        ...
        tippy.createSingleton(
            tippy(
                $(oissuEmbed)
                    .find(".os-toolbar [data-tippy-content]")
                    .toArray()
            ),
            {
                theme: "oissu oissu-button"
        ...
        let context = new Context(".os-t-chat", [
            ...
            },
            {
                ...
                callback: () => {
                    ...
                    oissu.config["custom"] = !oissu.config["custom"];
                    updateConfig();
                }
            },
            {
    ...
    function initImages(oissuEmbed) {
        ...
        if (display) {
            $.each(display.split(","), function() {
                $.getJSON(`https://r.tsukina.ga/i/${this}/_pack.json`, function(
                    data
                ) {
    ...
    function initConfig() {
        ...
        if (osck == null) {
            osck = JSON.stringify(oissu.config);
            localStorage.setItem("osck", osck);
        }
        oissu.config = JSON.parse(osck);
        // console.log(oissu.config);
        if (oissu.config["image"]) {
            $("body").addClass("oissu-image");
        }
        if (oissu.config["bubble"]) {
            $("body").addClass("oissu-bubble");
        }
        if (oissu.config["custom"]) {
            $("body").addClass("oissu-custom");
        }
    }

    function updateConfig() {
        localStorage.setItem("osck", JSON.stringify(oissu.config));
    }
